...
class ChannelFragment : Fragment() {
    ...
    private fun setupUser() {
        if (client.getCurrentUser() == null) {
            user = if (args.chatUser.username.contains("dhivya")) {
                User(
                    id = args.chatUser.username,
                    extraData = mutableMapOf(
                        "name" to args.chatUser.username,
                        "county" to "India",
                        "image" to "https://images.g2crowd.com/uploads/product/image/large_detail/large_detail_ba63e69c6c7fffd15743dbed662e33da/stream-chat.png"
                    )
                )
            } else {
                User(
    ...
    private fun setupDrawer() {
        ...
        headerId.text = currentUser.name
    }
